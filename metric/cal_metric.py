...
from metric.uiqm_function import getUIQM
...
def cal_batch_uiqm(pred):
    """

    :param pred: size (1, C, H, W)
    :return:
    """
    ...
    sum_uiqm = 0
    for i in range(pred.size(0)):
        cur_img = tensor2img(ten=pred[i])
        cur_uiqm = getUIQM(cur_img)
        sum_uiqm += cur_uiqm

    ave_uiqm = sum_uiqm / pred.size(0)
