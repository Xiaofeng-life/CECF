from methods.CECF.networks_CECF import AdaINGen_a, AdaINGen_b, MsImageDis
from methods.CECF.utils import weights_init, get_model_list, vgg_preprocess, get_scheduler
...
class CECF_Trainer(nn.Module):
    def __init__(self, hyperparameters, device):
        super(CECF_Trainer, self).__init__()
    ...
    def gen_update(self, x_a, x_b, hyperparameters):
        ...
        # add fix color loss
        fix_color_enhanced = self.gen_a.decode_fix(c_a, color_code)
        self.fix_color_loss = self.recon_criterion(fix_color_enhanced, x_b)
        if hyperparameters["use_ssim_loss"] == "Y":
            self.fix_color_loss_ssim = self.ssim_loss_func(fix_color_enhanced, x_b)
        ...
        # decode again (if needed), may not be used
        ...
        # total loss, the ``vgg_w'' always set to be zero
    ...
    def sample(self, x_a, x_b):
        ...
        for i in range(x_a.size(0)):
            ...
            # 新增
            x_a_fix_color.append(self.gen_a.decode_fix(c_a, color_code))
        ...
        self.train()
        return x_a, x_a_recon, x_ab1, x_ab2, x_a_fix_color, x_b, x_b_recon, x_ba1, x_ba2, x_a_fix_color
